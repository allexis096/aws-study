...
function efetuarLoginCognito(userName, password, callback) {
    ...
    getUser(userName).authenticateUser(authenticationDetails, tratarCallback(callback))
}
        ...
        cognitoUser.getUserAttributes((err, result) => {
            if (err) {
                ...
                for (let i = 0; i < result.length; i ++) {
                    userInfo[result[i].getName()] = result[i].getValue()
                }
...
function apagarUsuarioCognito(callback) {
    ...
    callback({ name: 'Erro', message: 'Usuário não está lodado' }, null)
}
